...
def build_fn(args, embeddings):
    ...
    elif args.att_func == 'wordbyword':
        att = nn_layers.multiAttentionRNN([network1, network2], args.rnn_output_size)
...
def main(args):
    ...
    if args.debug:
        ...
        test_examples = utils.load_data(args.test_file, args.max_test, relabeling=args.relabeling)
    ...
    for w in word_dict.keys():
        if len(w) ==1:
            word_dict_1[w] = word_dict[w]

    entity_markers = list(set([w for w in word_dict_1.keys()
    ...
    if args.test_only:  
        ...
        test_examples = utils.load_data(args.test_file, relabeling=args.relabeling)
        test_x1, test_x2, test_l, test_y = utils.vectorize(test_examples, word_dict, entity_dict,sort_by_len=False)
        all_test = gen_examples(test_x1, test_x2, test_l, test_y, 1)
        x1, mask1, x2, mask2, l, y = all_test[num]
        real = entity_dict.keys()[entity_dict.values().index(y[0])]
        pred = entity_dict.keys()[entity_dict.values().index(example_fn(x1, mask1,x2,mask2,l)[0])]
        print "Example # %s, True label: %s, predict: %s" %(num, real, pred)
    ...
    logging.info('params length is %d' %(len(params)))
    # Training
    logging.info('-' * 50)
...
if __name__ == '__main__':
    ...
    if args.test_file is None:
        raise ValueError('test_file is not specified.')
